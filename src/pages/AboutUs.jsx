...
const getIcon = (name, fallback = <FaAward />) => {
...
const AboutUs = ({ onOpenCallMe, onOpenEnquiry }) => {
  ...
  // Load from MongoDB backend and listen for local updates
  useEffect(() => {
    const fetchAboutData = async () => {
      try {
        ...
        console.warn('Backend offline, using local/cached About data:', err.message);
      }
    };
    ...
    const handleLocalUpdate = () => {
      try {
        const cached = localStorage.getItem('webmok_about_data');
        if (cached) setAboutData(JSON.parse(cached));
      } catch (e) {}
    };

    window.addEventListener('webmok_about_updated', handleLocalUpdate);
    return () => window.removeEventListener('webmok_about_updated', handleLocalUpdate);
  }, []);

  const bento = aboutData.bentoStats || defaultAboutData.bentoStats;
  const corporate = aboutData.corporateProfile || defaultAboutData.corporateProfile;
  const expertise = aboutData.expertiseExecution || defaultAboutData.expertiseExecution;
  const guidance = aboutData.executiveGuidance || defaultAboutData.executiveGuidance;

  // 4 Supported Business Types (Static Architecture Pillars)
  const businessTypes = [
    {
      icon: <FaHandshake />,
  ...
  // 8 Why Choose Us Pillars
  const whyChooseUsList = [
    { icon: <FaLaptopCode />, title: 'Talented App/Web Developers' },
    { icon: <FaUsers />, title: 'Specialized Workforce' },
    { icon: <FaClock />, title: 'Timely Delivery' },
    { icon: <FaHandshake />, title: 'Affordable Pricing' },
    { icon: <FaRegLightbulb />, title: 'Expert Advice' },
    { icon: <FaHeadset />, title: 'Valuable Customer Support' },
    { icon: <FaShieldAlt />, title: 'Finest Quality Service' },
    { icon: <FaCheckCircle />, title: 'A Trusted & Reliable Solution' }
  ];

  // 5 Values and Philosophies
  const valuesList = [
    { title: 'Positive & Ethical Relationships', desc: 'We cultivate positive, honest, and long-term ethical partnerships with every single client we serve.' },
    { title: 'Hard Work, Integrity & Trust', desc: 'We believe unreservedly in hard work, moral integrity, mutual trust, and shared commercial success.' },
    { title: 'Teamwork & Total Transparency', desc: 'We work as an integrated squad and maintain absolute, transparent communication with our users.' },
    { title: '100% Earned Result in Short Period', desc: 'We engineer high-efficiency sprints to deliver 100% verified, measurable results in accelerated timeframes.' },
    { title: 'Result-Oriented Execution', desc: 'We are fiercely outcome-focused, optimizing every technical action to yield superior business ROI.' }
  ];

  const currentServicesList = Array.isArray(expertise.services) && expertise.services.length > 0
    ? expertise.services
    : defaultAboutData.expertiseExecution.services;

  const activeSvc = currentServicesList[activeService] || currentServicesList[0] || {};
  ...
  return (
    <div className="wm-about-root">
      ...
      <section className="wm-about-hero">
        ...
        <div className="wm-about-container">
          <div className="wm-about-hero-content">
            <span className="wm-about-tag">KNOW ABOUT WEBMOK</span>
            <h1 className="wm-about-hero-title">
              About <span>Webmok</span>
            </h1>
            <p className="wm-about-hero-desc">
              Empowering businesses with innovative digital solutions, technology, creativity and global expertise.
            </p>
          </div>
        </div>
      </section>

      {/* 1.5 BENTO STATS SECTION (Hero k Just Niche Section - Fully Manageable) */}
      <section className="wm-about-bento-section">
        <div className="wm-about-container">
          <div className="wm-about-bento-grid">
            {/* Left: Tagline Column */}
            <div className="wm-bento-tagline-col">
              <span className="wm-bento-accent-word">{bento.accentWord}</span>
              <h2 className="wm-bento-tagline">{bento.tagline}</h2>
              <p className="wm-bento-tagline-desc">{bento.taglineDesc}</p>
              <div className="wm-bento-tagline-bar"></div>
            </div>

            {/* Dynamic 4 Stat Cards */}
            {Array.isArray(bento.stats) && bento.stats.map((st, i) => (
              <div key={st.id || i} className={`wm-bento-stat-card wm-bento-card-${i + 1}`}>
                <div className="wm-bento-card-overlay"></div>
                <div className="wm-bento-stat-content">
                  <span className="wm-bento-stat-num">{st.num}</span>
                  <span className="wm-bento-stat-label">{st.label}</span>
                </div>
                <div className="wm-bento-stat-icon">{getIcon(st.icon)}</div>
              </div>
            ))}

            {/* Far Right: Description Column */}
            <div className="wm-bento-desc-col">
              <span className="wm-about-sub" style={{ marginBottom: '14px', display: 'block' }}>
                {bento.whyTitle}
              </span>
              <p className="wm-bento-desc-para">{bento.whyDesc1}</p>
              <p className="wm-bento-desc-para">{bento.whyDesc2}</p>
      ...
      {/* 2. CORPORATE PROFILE SECTION (Fully Manageable) */}
      <section className="wm-about-profile-section">
        <div className="wm-about-container">
          <div className="wm-about-profile-grid">
            <div className="wm-about-profile-left">
              <span className="wm-about-sub">{corporate.subtitle}</span>
              <h2 className="wm-about-sec-title">{corporate.title}</h2>
              <div className="wm-about-bar"></div>

              <p className="wm-profile-lead-para">{corporate.leadPara}</p>
              <p className="wm-profile-para">{corporate.bodyPara}</p>
              ...
              <div className="wm-guarantee-box">
                ...
                <div className="wm-guarantee-content">
                  <h4>{corporate.guaranteeTitle}</h4>
                  <p>{corporate.guaranteeText}</p>
                </div>
              </div>
            </div>
            ...
            <div className="wm-about-profile-right">
              <div className="wm-profile-highlight-card">
                ...
                <ul className="wm-ph-list">
                  {Array.isArray(corporate.fastFacts) && corporate.fastFacts.map((fact, idx) => (
                    <li key={idx}>
      ...
      {/* 5. EXPERTISE & EXECUTION SECTION (Fully Manageable) */}
      <section className="wm-about-services-section">
        <div className="wm-about-container">
          <div className="wm-about-section-header">
            <span className="wm-about-sub">{expertise.subtitle}</span>
            <h2 className="wm-about-sec-title">{expertise.title}</h2>
            <div className="wm-about-bar"></div>
          </div>
              ...
              <div className="wm-svctab-scroll-wrap">
                {currentServicesList.map((svc, i) => (
                  ...
                  >
                    ...
                    <span className="wm-svctab-icon">{getIcon(svc.icon, <FaLaptopCode />)}</span>
                ...
                <div className="wm-svctab-panel" key={activeService}>
                  <div className="wm-svctab-panel-header">
                    <div className="wm-svctab-panel-iconbox">
                      {getIcon(activeSvc.icon, <FaLaptopCode />)}
                    </div>
                    <div>
                      <span className="wm-svctab-panel-num">{String(activeService + 1).padStart(2, '0')}</span>
                      <h3 className="wm-svctab-panel-title">{activeSvc.title}</h3>
                    </div>
                  </div>

                  <p className="wm-svctab-panel-desc">{activeSvc.desc}</p>

                  <div className="wm-svctab-features-grid">
                    {Array.isArray(activeSvc.subFeatures) && activeSvc.subFeatures.map((feat, fi) => (
      ...
      {/* 7. EXECUTIVE GUIDANCE SECTION (Leadership Team - Fully Manageable) */}
      <section className="wm-leadership-section">
        <div className="wm-about-container">
          <div className="wm-about-section-header">
            <span className="wm-about-sub">{guidance.subtitle}</span>
            <h2 className="wm-about-sec-title">{guidance.title}</h2>
            <div className="wm-about-bar"></div>
          </div>

          <div className="wm-team-grid">
            {Array.isArray(guidance.leaders) && guidance.leaders.map((leader, i) => (
              <div key={leader.id || i} className="wm-team-card">
                <div className="wm-team-img-wrap">
                  <img src={getMediaUrl(leader.image)} alt={leader.name} className="wm-team-img" />
                </div>
